...
export class CalcLib {
     ...
     */
    static angleBetweenVectors(v1: p5.Vector, v2: p5.Vector, doRound = true, roundDec = 2) {
        let res = v1.angleBetween(v2);

        return (doRound) ? p5.round(res, roundDec) : res;
    }

    static distBetweenCircleVectors(r: number, v1: p5.Vector, v2: p5.Vector) {
        // circumference
        let C = 2 * p5.PI * r;
    ...
    static distBetweenCircleVectorsUsingAngle(r: number, angle: number) {
        // circumference
        let C = 2 * p5.PI * r;
